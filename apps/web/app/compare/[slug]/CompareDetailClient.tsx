  ...
  if (!data) {
    return (
      <div className="container mx-auto px-4 lg:px-6 py-16 text-center">
        ...
        <Link href="/compare" className="text-white hover:text-[#deff0a] transition inline-flex items-center gap-1">
      ...
      <div className="container mx-auto px-4 lg:px-6 py-8 max-w-5xl relative">
          ...
          <div className="text-stone-300 space-y-3">
            ...
            {data.priceWinner && (
              <p className="flex items-start gap-2">
                <Trophy className="w-5 h-5 text-white mt-0.5 shrink-0" weight="fill" />
                <span>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Link
            ...
            className="flex items-center justify-center gap-2 bg-white hover:bg-[#ccff00] text-stone-900 font-semibold py-3 px-6 rounded-xl transition shadow-lg shadow-[#deff0a]/20 hover:shadow-[#deff0a]/30"
          >
            View {pluginA.name}
...
function PluginCard({ plugin, isWinner }: { plugin: { name: string; manufacturer: string; description?: string; price: string; isFree: boolean; imageUrl?: string; productUrl: string; tags: string[] }; isWinner: boolean }) {
  return (
    <div className={`glass-card rounded-xl p-6 ${isWinner ? '!border-[#deff0a]/40' : ''}`}>
      {isWinner && (
        <div className="text-white text-sm font-semibold mb-3 flex items-center gap-1">
...
function ComparisonRow({
  ...
  return (
    <div className="grid grid-cols-3 p-4 border-t border-white/[0.06]">
      ...
      <div className={`text-center ${winner === 'a' ? 'text-white font-semibold' : 'text-stone-100'}`}>
        {valueA}
      </div>
      <div className={`text-center ${winner === 'b' ? 'text-white font-semibold' : 'text-stone-100'}`}>
        {valueB}
      </div>
    </div>
  );
