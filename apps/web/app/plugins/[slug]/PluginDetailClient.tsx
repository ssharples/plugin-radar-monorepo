...
import { PluginPresetsSection } from "@/components/presets/PluginPresetsSection";
...
export default function PluginDetailClient({ slug, initialPlugin }: { slug: string; initialPlugin: any }) {
  const pluginQuery = useQuery(api.plugins.getBySlugWithManufacturer, slug ? { slug } : "skip");
  const pluginData = pluginQuery ?? initialPlugin;
            ...
            <div className="relative rounded-2xl overflow-hidden bg-[#1e1b18] md:sticky md:top-24">
              ...
              <div className="absolute top-3 right-3">
                <Link
                  ...
                  className="px-2.5 py-1 bg-black/50 backdrop-blur-sm rounded-lg text-[11px] hover:brightness-125 transition capitalize"
                  style={{ color: getCategoryColor(plugin.category) ?? '#d6d3d1' }}
                >
                  {plugin.category}
                </Link>
              </div>
