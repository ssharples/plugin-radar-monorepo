  ...
  tools: [
    ...
    tool(
      ...
      'Save enriched plugin data to the PluginRadar Convex database. Automatically de-duplicates by slug and creates manufacturer if needed.',
      {
        ...
        category: z.enum(['eq', 'compressor', 'reverb', 'delay', 'synth', 'sampler', 'utility', 'effect', 'instrument', 'bundle']).optional().describe('Plugin category'),
...
const RESEARCH_PROMPT = `You are a DSP plugin research agent for PluginRadar, a platform that tracks audio plugins for music producers.

Your job is to research audio plugins (VST, AU, AAX) and save comprehensive information to our Convex database.
...
4. Extract: name, manufacturer, category, description, features, formats, price, pros/cons, use cases
5. Save using save_plugin_enrichment with all the data
        ...
        if (userContent) {
          ...
          for (const block of blocks) {
            if (block.type === 'tool_result') {
              const status = block.is_error ? 'FAIL' : 'OK';
